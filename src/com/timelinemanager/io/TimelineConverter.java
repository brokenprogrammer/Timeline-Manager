...
public class TimelineConverter {
	...
	private static final String[] TIMELINE_FIELDS = {"[Timeline]", "Title:", "Description:", "Start Date:", "End Date:"};
	private static final String[] EVENT_FIELDS = {"[Event]", "Title:", "Description:", "Start Date:", "End Date:", "Start Time:", "End Time:"};
	...
	/**
	 * Checks if the given file is a valid timeline file by
	 * checking if all the required fields for a timeline is stored within the file.
	 * 
	...
	/**
	 * Reads the timeline from a String array. A timeline has required fields and
	 * all of them have to be added to the timeline so this method throws an error 
	 * if there is a missing field.
	 *  
	 * @param content - Array with data to populate the Timeline data.
	 * @param pos - Position within the content array to start from.
	 * @throws IllegalArgumentException - When a field is missing.
	 * @return String array with data content to use when making a timeline object.
	 */
	private static String[] readTimeline(String[] content, int pos) {
		//Data content to be extracted for a timeline, -1 because [Timeline] does not count.
	 ...
	 * 
	 * @param content - Array with data to populate the Event data.
	 * @param pos - Position within the content array to start from.
	...
	private static Event readEvent(String[] content, int pos) {
		//Data content to be extracted for a event, -1 because [Event] does not count.
		...
		newEvent.setStartDate(LocalDate.parse(s[2]));
		if (!s[3].equals("") && s[3] != null) {
			newEvent.setEndDate(LocalDate.parse(s[3]));
		}
		if (!s[4].equals("") && s[4] != null) {
		newEvent.setStartTime(LocalTime.parse(s[4]));
		}
		if (!s[5].equals("") && s[5] != null) {
		newEvent.setEndTime(LocalTime.parse(s[5]));
		}
