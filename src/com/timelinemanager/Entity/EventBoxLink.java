	...
	public void setEventBoxLink(LocalDateTime start, int span, boolean dayLevel, ArrayList<Event> inbigArr) {
		...
		if (dayLevel == true) {
			...
			for (int m = 0; m < span; m++) {
				...
				for (int i = 0; i < shortEvent.size(); i++) {
					if (shortEvent.get(i).getStartDate().getMonth().getValue() == temp.getMonth().getValue()
							&& shortEvent.get(i).getEndDate().getMonth().getValue() == temp.getMonth().getValue()) {
						sb.append(shortEvent.get(i).getTitle() + ".\n");
					}
				}
				temp = temp.plusMonths(1);

				if (sb.length() == 0) {
					tool.setText("No short events during this month");
				}
				if (sb.length() > 0) {
					String msg = "Events during this month:\n" + sb.toString();
					tool.setText(msg);
				}
			...
			for (int i = 0; i < arr.size(); i++) {
				...
				while (temp1.isBefore(a.getStart())) {
					int monthLength = temp1.getMonth().length(temp1.toLocalDate().isLeapYear());
				...
				if (i == 0) {
					GridPane.setMargin(a, new Insets(0, 0, 0, pos));
					this.add(a, 0, 0);
				}
				if (i > 0) {
					if (a.getStart().isAfter(arr.get(i - 1).getStart())
